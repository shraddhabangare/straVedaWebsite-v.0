...
export default function StravedaHeroBackground() {
  ...
  // ── Mouse-following via GSAP ticker — shares the same RAF as Lenis ──
  // IntersectionObserver fully STOPS the ticker callback when off-screen,
  // eliminating the competing RAF that was stealing frame budget from Lenis.
  useEffect(() => {
    const tg  = { x: 0, y: 0 };
    const cur = { x: 0, y: 0 };
    let visible = true;
    ...
    // Add/remove from GSAP ticker based on visibility so we never
    // waste a frame when the hero is off-screen.
    ...
    // Start immediately (hero is visible on load)
    gsap.ticker.add(tick);
